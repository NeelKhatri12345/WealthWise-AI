...
export const ReportGenerator = ({ onGenerate, isLoading = false }: ReportGeneratorProps) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
            {errors.dateFrom && <p className="mt-1 text-xs text-red-600">{errors.dateFrom.message}</p>}
          </div>
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500"
            />
            {errors.dateTo && <p className="mt-1 text-xs text-red-600">{errors.dateTo.message}</p>}
          </div>
        </div>

        <div className="flex gap-4">
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input type="checkbox" {...register('includeCharts')} className="rounded text-indigo-600" />
            Include Charts
          </label>
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input type="checkbox" {...register('includeSummary')} className="rounded text-indigo-600" />
            Include Summary
          </label>
        </div>

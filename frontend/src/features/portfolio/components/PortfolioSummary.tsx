    ...
    <div className="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white shadow-lg">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-3xl font-bold">${totalValue.toLocaleString()}</p>
          <div className={`mt-2 flex items-center gap-1 text-sm ${isPositive ? 'text-green-300' : 'text-red-300'}`}>
        ...
        <div className="text-right">
          ...
          {lastUpdated && (
            <p className="mt-1 text-xs text-indigo-300">Updated: {lastUpdated}</p>
          )}
        </div>
      </div>
    </div>

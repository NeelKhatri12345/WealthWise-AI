...
export const fetchHealthScore = createAsyncThunk('healthScore/fetchScore', async (_, { rejectWithValue }) => {
    try {
        ...
        const error = err;
        return rejectWithValue(error.response?.data?.message ?? 'Failed to fetch health score');
    }
});
export const fetchHealthHistory = createAsyncThunk('healthScore/fetchHistory', async (period = '6m', { rejectWithValue }) => {
    try {
        const { healthApi } = await import('../../services/api/health.api');
